...
#pragma warning restore 649
        ...
        private void StartWallrun()
        {
            ...
            {
                _rigidbody.AddForce(orientation.forward * wallrunForce * Time.deltaTime);

                //Make sure char sticks to wall
                if (isWallRight)
                    _rigidbody.AddForce(orientation.right * wallrunForce / 5 * Time.deltaTime);
                else
                    _rigidbody.AddForce(-orientation.right * wallrunForce / 5 * Time.deltaTime);
            }
        }
        ...
        private void RotateCameraAndCharacter()
        {
            ...
            var rotation = arms.rotation *
                           ...
                           Quaternion.AngleAxis(clampedY, Vector3.left); //* Quaternion.AngleAxis(maxWallRunCameraTilt, Vector3.forward);
        ...
        private void Jump()
        {
            if ((!_isGrounded && !_canDoubleJump) || !input.Jump) return;
            if (!_isGrounded && _canDoubleJump) _canDoubleJump = false;
            _isGrounded = false;
            ...
            if (isWallRunning)
            {
                ...
                //sidwards wallhop
                if (isWallRight || isWallLeft && Input.GetKey(KeyCode.A) || Input.GetKey(KeyCode.D)) _rigidbody.AddForce(-orientation.up * jumpForce * 1f);
                if (isWallRight && Input.GetKey(KeyCode.A)) _rigidbody.AddForce(-orientation.right * jumpForce * 3.2f);
                if (isWallLeft && Input.GetKey(KeyCode.D)) _rigidbody.AddForce(orientation.right * jumpForce * 3.2f);

                //Always add forward force
                _rigidbody.AddForce(orientation.forward * jumpForce * 1f);
