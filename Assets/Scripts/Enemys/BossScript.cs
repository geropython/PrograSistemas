...
public class BossScript : MonoBehaviour
{
            ...
            if (Vector3.Distance(transform.position, target.transform.position) > 1 && !atacando)
            {
                switch (rutina)
                {
                    case 0:
                        ...
                        if(!onlyShoot) ani.SetBool("Walk", true);

                        ani.SetBool("Run", false);

                        if (transform.rotation == rotation)
                        {
                            if(!onlyShoot) transform.Translate(Vector3.forward * speed * Time.deltaTime);
                        }
                        ...
                        {
                            if (!onlyShoot && !onlyMelee) rutina = Random.Range(0, 3);
                            else if (onlyShoot) rutina = 2;
                            else if (onlyMelee) rutina = Random.Range(0, 2);
                    ...
                    case 2:
                        ...
                        transform.rotation = Quaternion.RotateTowards(transform.rotation, rotation, 2);
                        if(!onlyShoot) rango.GetComponent<CapsuleCollider>().enabled = false;
                        break;
                    /*case 3:
                        //jump//
                        if (fase == 2)
    ...
    public void Final_ani()
    {
        ...
        ani.SetBool("Attack", false);
        atacando = false;
        if(!onlyShoot)rango.GetComponent<CapsuleCollider>().enabled = true;
        //lanza_llamas = false;
        //jump_distance = 0;
    ...
    public void FireBallSkill()
    {
        ...
        obj.GetComponent<CustomBullets>().rb.velocity = obj.transform.forward * fireballSpeed;
